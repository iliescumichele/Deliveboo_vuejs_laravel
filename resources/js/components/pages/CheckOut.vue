            ...
            <div class="user-detail-container bg-light">
                ...
                <ValidationObserver v-slot="{ handleSubmit }">
                    <form @submit.prevent="handleSubmit(onSubmit)" class="p-3" id="checkout-form">
                        ...
                        <validationProvider name="indirizzo" rules="required|alpha_num|min:5" v-slot="{ errors }" class="form-group">
...
export default {
  ...
  methods: {
    onSubmit(){
        ...
        if( patternNumber.test(this.orderCustomerInfo.telephone_number) === true &&
            this.orderCustomerInfo.address !="" &&
            this.orderCustomerInfo.name !="" &&
            this.orderCustomerInfo.surname !="" &&
            ( this.orderCustomerInfo.email.includes('@') && this.orderCustomerInfo.email.includes('.', this.orderCustomerInfo.email.indexOf('@')) && (patternDomain.test(domain) === true && domain.length > 1))
        ) {
